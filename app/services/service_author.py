...
from app.repositories.mongodb_repo.repo_author import AuthorRepository
...
class AuthorService:
    ...
    async def get_authors_by_user_id(self, user_id: uuid.UUID):
        return await self.repo.get_all_by_user_id(user_id)

    async def get_author_by_id(self, author_id: uuid.UUID):
        return await self.repo.get_by_id(author_id)

    async def get_author_by_id_and_user(self, author_id: uuid.UUID, user_id: uuid.UUID):
        return await self.repo.get_by_id_and_user_id(author_id, user_id)

    async def create_author(self, author_in: AuthorCreate):
        return await self.repo.create(author_in.model_dump())

    async def update_author(self, author_id: uuid.UUID, user_id: uuid.UUID, author_in: AuthorUpdate):
